...
class LiteBansSpider(scrapy.Spider):
    ...
    def find_indices(self, headers):
        ...
        """
        translated_headers = [self.translate_header('reason', header) or self.translate_header('date', header) or self.translate_header('expires', header) for header in headers]

        # Find the indices of the cells we're interested in
        reason_index = translated_headers.index('reason') if 'reason' in translated_headers else None
        date_index = translated_headers.index('date') if 'date' in translated_headers else None
        expiry_index = translated_headers.index('expires') if 'expires' in translated_headers else None
        ...
        return BanItem(
            {
                ...
                "date": calculate_timestamp(
                    parse_date(columns[date_index].text.strip())
                ),
                "expires": calculate_timestamp(
                    parse_date(columns[expiry_index].text.strip())
                ),
                "url": url,
            }
        )
