import json
import utils
from webpages import litebans, mcbouncer, mcbans, mconline, johnmuffin, mccentral, cosmicgames, mcbrawl
...
async def main():
    async with ClientSession(headers=get_headers()) as session:
        for url_type in external_urls:
            ...
            if url_type == 'MCBRAWL':
                MCBrawlBans = await asyncio.gather(*[mcbrawl.handle_request(url.replace("<USERNAME>", PLAYER_USERNAME), session) for url in external_urls[url_type]])
                print(MCBrawlBans)
                # print(f"MCBrawl: {[ban for ban in MCBrawlBans[0] if ban is not None and len(ban) > 0]}\n")
            ...
            # if url_type == 'SNAPCRAFT':
            #     print(f"SnapCraft: {external_urls[url_type]}\n")
